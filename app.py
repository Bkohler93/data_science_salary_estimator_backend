...
def send_attribute_names():
    with engine.connect() as conn:
        ...
        exp_lvl_stmt = text(
            "SELECT experience_level FROM ( SELECT experience_level, COUNT(*) AS field_count FROM salaries GROUP BY experience_level) AS counted_fields ORDER BY field_count DESC")
        emp_type_stmt = text(
            "SELECT employment_type FROM ( SELECT employment_type, COUNT(*) AS field_count FROM salaries GROUP BY employment_type) AS counted_fields ORDER BY field_count DESC")
        job_title_stmt = text(
            "SELECT job_title FROM ( SELECT job_title, COUNT(*) AS field_count FROM salaries GROUP BY job_title) AS counted_fields ORDER BY field_count DESC")
        emp_res_stmt = text(
            "SELECT employee_residence FROM ( SELECT employee_residence, COUNT(*) AS field_count FROM salaries GROUP BY employee_residence) AS counted_fields ORDER BY field_count DESC")
        comp_loc_stmt = text(
            "SELECT company_location FROM ( SELECT company_location, COUNT(*) AS field_count FROM salaries GROUP BY company_location) AS counted_fields ORDER BY field_count DESC")
        comp_size_stmt = text(
            "SELECT company_size FROM ( SELECT company_size, COUNT(*) AS field_count FROM salaries GROUP BY company_size) AS counted_fields ORDER BY field_count DESC")
...
def send_avg_salary_data():
    ...
    with engine.connect() as conn:
        stmt = select(salary_table.c[field], func.avg(salary_table.c.salary).label('salary')).where(
            salary_table.c.job_title == job_title).group_by(salary_table.c[field]).limit(7)
